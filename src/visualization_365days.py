...
def visualize_forecast():
    ...
    daily_totals.columns = ['date', 'total_visitors']
    
    # Create figure with multiple subplots
    fig = plt.figure(figsize=(20, 12))
    
    # ============= PLOT 1: Daily Visitors Over Time =============
    ax1 = plt.subplot(3, 2, 1)
    ax1.plot(daily_totals['date'], daily_totals['total_visitors'], 
             linewidth=2, color='#2E86AB', alpha=0.8)
    ax1.fill_between(daily_totals['date'], daily_totals['total_visitors'], 
    ...
    ax1.scatter(daily_totals[christmas_mask]['date'], 
               daily_totals[christmas_mask]['total_visitors'],
               color='red', s=100, zorder=5, label='Christmas Week', alpha=0.7)
    
    ax1.set_title('Daily Visitor Forecast - 365 Days', fontsize=14, fontweight='bold')
    ...
    # Add average line
    avg_daily = daily_totals['total_visitors'].mean()
    ...
    ax2.set_title('Monthly Visitor Totals', fontsize=14, fontweight='bold')
    ax2.set_xlabel('Month', fontsize=11)
    ax2.set_ylabel('Total Visitors', fontsize=11)
    ...
    daily_totals['7d_avg'] = daily_totals['total_visitors'].rolling(window=7, center=True).mean()
    daily_totals['30d_avg'] = daily_totals['total_visitors'].rolling(window=30, center=True).mean()
    ...
    ax3.set_ylabel('Average Daily Visitors', fontsize=11)
    ax3.legend()
    ...
    dow_avg = daily_totals.groupby('day_of_week')['total_visitors'].mean()
    ...
    ax4.set_title('Average Visitors by Day of Week', fontsize=14, fontweight='bold')
    ax4.set_xlabel('Day of Week', fontsize=11)
    ax4.set_ylabel('Average Visitors', fontsize=11)
    ...
    ax5.set_title('Daily Visitors by Ticket Family', fontsize=14, fontweight='bold')
    ax5.set_xlabel('Date', fontsize=11)
    ax5.set_ylabel('Daily Visitors', fontsize=11)
    ax5.legend()
    ...
    total_visitors = daily_totals['total_visitors'].sum()
    avg_daily_visitors = daily_totals['total_visitors'].mean()
    max_day = daily_totals.loc[daily_totals['total_visitors'].idxmax()]
    min_day = daily_totals.loc[daily_totals['total_visitors'].idxmin()]
    
    # Christmas week stats
    christmas_data = daily_totals[christmas_mask]
    christmas_avg = christmas_data['total_visitors'].mean() if not christmas_data.empty else 0
    
    # Low season stats
    low_season_mask = daily_totals['date'].dt.month.isin([1, 2, 11])
    low_season_avg = daily_totals[low_season_mask]['total_visitors'].mean()
    
    # Peak season stats
    peak_season_mask = daily_totals['date'].dt.month.isin([7, 8])
    peak_season_avg = daily_totals[peak_season_mask]['total_visitors'].mean()
    
    metrics_text = f"""KEY FORECAST METRICS

OVERALL STATISTICS
Total Predicted Visitors: {int(total_visitors):,}
Average Daily Visitors: {int(avg_daily_visitors):,}

EXTREMES
Highest Day: {max_day['date'].strftime('%Y-%m-%d')}
             {int(max_day['total_visitors']):,} visitors

Lowest Day:  {min_day['date'].strftime('%Y-%m-%d')}
             {int(min_day['total_visitors']):,} visitors
...
"""
    ...
    # Overall title
    fig.suptitle('365-Day Visitor Forecast Analysis', 

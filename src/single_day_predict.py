import os
import pickle
import warnings
import numpy as np
import pandas as pd
from datetime import datetime, timedelta
from paths import MODELS_DIR, PROCESSED_DATA_PATH, HOLIDAY_DATA_PATH

warnings.filterwarnings("ignore")

# =============================================================================
# 1. HELPER FUNCTIONS
# =============================================================================

def _safe_float(x, default=0.0):
    try:
        return float(x) if not pd.isna(x) else default
    except:
        return default

def _compute_extreme_multipliers(processed_df: pd.DataFrame) -> dict:
    baseline_months = [3, 4, 5, 9, 10]
    baseline_data = processed_df[processed_df['date'].dt.month.isin(baseline_months)]
    
    if baseline_data.empty:
        baseline_avg = processed_df.groupby('date')['ticket_num'].sum().median()
    else:
        baseline_avg = baseline_data.groupby('date')['ticket_num'].sum().median()
        
    if baseline_avg == 0: baseline_avg = 1.0

    recent_df = processed_df[processed_df['date'].dt.year >= 2022]
    xmas_data = recent_df[
        (recent_df['date'].dt.month == 12) & 
        (recent_df['date'].dt.day.between(27, 30))
    ]
    
    if not xmas_data.empty:
        xmas_avg = xmas_data[xmas_data['ticket_num'] > 0].groupby('date')['ticket_num'].sum().mean()
    else:
        xmas_avg = baseline_avg * 3.0

    low_data = processed_df[processed_df['date'].dt.month.isin([1, 2, 11])]
    low_avg = low_data.groupby('date')['ticket_num'].sum().mean() if not low_data.empty else baseline_avg * 0.3

    return {
        'baseline': float(baseline_avg),
        'xmas_multiplier': float(xmas_avg / baseline_avg),
        'low_season_multiplier': float(low_avg / baseline_avg)
    }

def _get_adaptive_scaling_bounds(current_date: datetime, fam_wape: float) -> tuple[float, float]:
    month = current_date.month
    day = current_date.day
    is_trusted = fam_wape < 15.0

    if month == 12 and 25 <= day <= 31:
        return (0.8, 8.0)
    elif month in [1, 2, 11]:
        return (0.0, 1.5)
    else:
        return (0.70, 1.50) if is_trusted else (0.50, 1.80)

def _apply_extreme_shape_injection(pred: float, current_date: datetime, multipliers: dict) -> float:
    month = current_date.month
    day = current_date.day
    baseline = multipliers['baseline']

    if month == 12 and 21 <= day <= 24:
        ramp_progress = (day - 21) / 3.0
        ramp_mult = 1.5 + (ramp_progress * 1.5)
        ideal_val = baseline * ramp_mult
        
        if pred < ideal_val:
            weight = 0.9
            injected = (pred * (1 - weight)) + (ideal_val * weight)
            return min(injected, 6000.0)

    elif month == 12 and 26 <= day <= 30:
        target_mult = multipliers['xmas_multiplier']
        ideal_val = (baseline * target_mult) * 1.3
        
        if pred < ideal_val:
            progress = min(max((day - 26) / 3.0, 0.0), 1.0)
            weight = 0.5 + (progress * 0.4)
            injected = (pred * (1 - weight)) + (ideal_val * weight)
            return min(injected, ideal_val * 1.1)

    return pred

def _choose_model(ticket_family, global_model, family_models, global_wape, family_wape_map):
    fam_model = family_models.get(ticket_family)
    if fam_model is None: return global_model
    
    fam_wape = family_wape_map.get(ticket_family, np.inf)
    if np.isfinite(global_wape) and fam_wape < global_wape:
        return fam_model
    if fam_wape < 50.0:
        return fam_model
    return global_model

# =============================================================================
# 2. LOAD RESOURCES (Run Once)
# =============================================================================
print("Loading models and data...")

with open(MODELS_DIR / "lgbm_model.pkl", "rb") as f: global_model = pickle.load(f)
with open(MODELS_DIR / "family_models.pkl", "rb") as f: family_models = pickle.load(f)
with open(MODELS_DIR / "feature_cols.pkl", "rb") as f: model_features = pickle.load(f)

try:
    with open(MODELS_DIR / "model_performance.pkl", "rb") as f: perf = pickle.load(f)
    global_wape = _safe_float(perf.get("global_model", {}).get("WAPE", np.inf), np.inf)
    family_wape_map = {k: _safe_float(v.get("WAPE", np.inf), np.inf) for k, v in perf.get("family_models_heldout", {}).items()}
except:
    global_wape, family_wape_map = np.inf, {}

processed_df = pd.read_csv(PROCESSED_DATA_PATH)
processed_df["date"] = pd.to_datetime(processed_df["date"], errors="coerce").dt.normalize()
processed_df = processed_df.dropna(subset=["date"]).copy()

ticket_list = (
    processed_df[["ticket_name", "ticket_family"]]
    .drop_duplicates()
    .sort_values(["ticket_family", "ticket_name"])
    .itertuples(index=False, name=None)
)
families = sorted(processed_df["ticket_family"].dropna().unique().tolist())

meta_cols = ["ticket_name", "ticket_family", "groupID", "is_actie_ticket", "is_abonnement_ticket", 
             "is_full_price", "is_accommodation_ticket", "is_group_ticket", "is_joint_promotion"]
ticket_meta = processed_df[[c for c in meta_cols if c in processed_df.columns]].drop_duplicates("ticket_name").set_index("ticket_name")

history_feature_cols = [c for c in model_features if "lag" in c or "rolling" in c or "sales" in c or "available" in c]
hist_by_ticket = {}
print("Building history lookup...")
for tname, _ in ticket_list:
    tdf = processed_df.loc[processed_df["ticket_name"] == tname, ["date"] + history_feature_cols]
    hist_by_ticket[tname] = tdf.set_index("date").sort_index() if not tdf.empty else pd.DataFrame()

try:
    holiday_og_df = pd.read_excel(HOLIDAY_DATA_PATH)
    holiday_dates = pd.to_datetime(holiday_og_df.iloc[:, 5], errors="coerce").dropna().unique()
    holiday_dates_sorted = np.array(sorted(holiday_dates), dtype="datetime64[ns]")
except:
    holiday_dates_sorted = np.array([], dtype="datetime64[ns]")

daily_total = processed_df.groupby("date")["ticket_num"].sum().reset_index(name="total")
daily_total["doy"] = daily_total["date"].dt.dayofyear
doy_means = daily_total.groupby("doy")["total"].mean().reset_index()
doy_means["smooth_total"] = doy_means["total"].rolling(window=7, center=True, min_periods=1).mean()
raw_target_doy_map = doy_means.set_index("doy")["smooth_total"].to_dict()

fam_daily = processed_df.groupby(["date", "ticket_family"])["ticket_num"].sum().reset_index(name="fam_total")
fam_daily["doy"] = fam_daily["date"].dt.dayofyear
raw_target_family_map = {}
for fam in families:
    f_df = fam_daily[fam_daily["ticket_family"] == fam]
    f_means = f_df.groupby("doy")["fam_total"].mean().reset_index()
    f_means["smooth_total"] = f_means["fam_total"].rolling(window=7, center=True, min_periods=1).mean()
    for _, r in f_means.iterrows():
        raw_target_family_map[(fam, int(r["doy"]))] = float(r["smooth_total"])

bt = processed_df[processed_df["date"] >= (processed_df["date"].max() - pd.Timedelta(days=30))]
recent_actuals = bt["ticket_num"].sum()
recent_historical_baseline = sum(raw_target_doy_map.get(d.dayofyear, 0) for d in bt["date"].unique()) or 1.0
trend_ratio = np.clip(recent_actuals / recent_historical_baseline, 0.70, 1.30)
extreme_multipliers = _compute_extreme_multipliers(processed_df)

# =============================================================================
# 3. SINGLE DAY PREDICTION FUNCTION (UPDATED)
# =============================================================================

def predict_single_day(
    date: str,
    temperature: float,
    rain_morning: float,
    rain_afternoon: float,
    precip_morning: float,
    precip_afternoon: float,
    event_name: str = None,
    holiday_name: str = None,
    holiday_intensity: int = 0,
    manual_days_until_holiday: int = None,
    manual_days_since_holiday: int = None
):
    """
    Predicts sales for a single day using separate Morning/Afternoon inputs.
    """
    date = pd.to_datetime(date).normalize()
    year, month, day = date.year, date.month, date.day
    doy = date.dayofyear
    
    # Calculate totals for features that require the sum
    total_rain = rain_morning + rain_afternoon
    total_precipitation = precip_morning + precip_afternoon

    print(f"\nGenerating prediction for: {date.date()}")
    print(f"Weather: {temperature}°C | Rain: {rain_morning}/{rain_afternoon} | Precip: {precip_morning}/{precip_afternoon}")

    if manual_days_until_holiday is not None:
        days_until = manual_days_until_holiday
        days_since = manual_days_since_holiday if manual_days_since_holiday else 90
    else:
        if holiday_dates_sorted.size == 0:
            days_until, days_since = 90, 90
        else:
            d_np = np.datetime64(date)
            pos = np.searchsorted(holiday_dates_sorted, d_np, side="left")
            if pos >= holiday_dates_sorted.size:
                next_h = d_np + np.timedelta64(90, "D")
            else:
                next_h = holiday_dates_sorted[pos]
            if pos == 0:
                prev_h = d_np - np.timedelta64(90, "D")
            else:
                prev_h = holiday_dates_sorted[pos - 1]
            days_until = int((next_h - d_np) / np.timedelta64(1, "D"))
            days_since = int((d_np - prev_h) / np.timedelta64(1, "D"))

    per_ticket_results = []
    per_family_tot = {fam: 0.0 for fam in families}
    yoy_date = (date - timedelta(days=364)).normalize()

    for ticket_name, ticket_family in ticket_list:
        feat = {
            "year": year, "month": month, "day": day, 
            "week": date.isocalendar().week, "weekday": date.weekday(),
            "day_of_year": doy, 
            "is_weekend": int(date.weekday() >= 5),
            "temperature": temperature,
            
            # --- UPDATED MANUAL INPUTS ---
            "rain_morning": rain_morning,
            "rain_afternoon": rain_afternoon,
            "precip_morning": precip_morning,
            "precip_afternoon": precip_afternoon,
            "total_rain": total_rain,
            "total_precipitation": total_precipitation,
            
            "days_until_holiday": days_until,
            "days_since_holiday": days_since,
            "holiday_intensity": holiday_intensity,
        }

        meta = ticket_meta.loc[ticket_name] if ticket_name in ticket_meta.index else None
        if meta is not None:
            for c in meta.index:
                if c not in ["ticket_name", "ticket_family"]:
                    feat[c] = meta[c]
        
        feat[f"ticket_{ticket_name}"] = 1
        feat[f"family_{ticket_family}"] = 1
        
        yoy_vals = hist_by_ticket.get(ticket_name, pd.DataFrame()).asof(yoy_date)
        if isinstance(yoy_vals, pd.Series):
            for k, v in yoy_vals.items():
                if k not in feat: feat[k] = v
        
        for col in model_features:
            if col.startswith("event_"):
                feat[col] = 1 if event_name and col == f"event_{event_name}" else 0
            if "holiday" in col and ("region" in col or "_" in col):
                if holiday_name and holiday_name.lower() in col.lower():
                     feat[col] = 1
                elif col not in feat:
                     feat[col] = 0

        X = pd.DataFrame([feat]).reindex(columns=model_features, fill_value=0)
        
        model = _choose_model(ticket_family, global_model, family_models, global_wape, family_wape_map)
        pred_raw = float(model.predict(X)[0])
        pred_val = max(0.0, pred_raw)
        
        per_ticket_results.append((ticket_name, ticket_family, pred_val))
        per_family_tot[ticket_family] += pred_val

    # Scaling Logic
    final_scaled = []
    family_scaled_tot = {}
    for fam in families:
        fam_pred = per_family_tot[fam]
        fam_target = float(raw_target_family_map.get((fam, doy), fam_pred)) * trend_ratio
        if fam_pred <= 0:
            family_scaled_tot[fam] = fam_pred
            continue
        fam_wape = float(family_wape_map.get(fam, 25.0))
        lo, hi = _get_adaptive_scaling_bounds(date, fam_wape)
        ratio = np.clip(fam_target / fam_pred, lo, hi)
        family_scaled_tot[fam] = fam_pred * ratio

    temp_scaled_list = []
    for tname, tfam, pred in per_ticket_results:
        fam_pred = per_family_tot[tfam]
        fam_sc = family_scaled_tot[tfam]
        val = pred * (fam_sc / fam_pred) if fam_pred > 0 else pred
        temp_scaled_list.append((tname, tfam, val))

    day_pred = sum(x[2] for x in temp_scaled_list)
    day_target = float(raw_target_doy_map.get(doy, day_pred)) * trend_ratio
    
    if day_pred > 0:
        lo, hi = _get_adaptive_scaling_bounds(date, 10.0)
        ratio = np.clip(day_target / day_pred, lo, hi)
        if month == 12 and (21 <= day <= 24 or 27 <= day <= 30) and ratio < 1.0:
            ratio = 1.0
        final_scaled = [(t, f, v * ratio) for t, f, v in temp_scaled_list]
        day_pred = day_pred * ratio
    else:
        final_scaled = temp_scaled_list

    final_total = _apply_extreme_shape_injection(day_pred, date, extreme_multipliers)
    if day_pred > 0 and final_total != day_pred:
        global_ratio = final_total / day_pred
        final_scaled = [(t, f, v * global_ratio) for t, f, v in final_scaled]

    final_modifier = 1.2 if (date.weekday() >= 5) else 0.9
    final_scaled = [(t, f, v * final_modifier) for t, f, v in final_scaled]

    total_sales = sum(x[2] for x in final_scaled)
    print(f"\n{'='*40}")
    print(f"PREDICTION RESULT")
    print(f"{'='*40}")
    print(f"Date: {date.date()}")
    print(f"Total Predicted Sales: {int(total_sales):,}")
    print(f"{'-'*40}")
    
    fam_results = {}
    for _, fam, val in final_scaled:
        fam_results[fam] = fam_results.get(fam, 0) + val
    
    for fam, val in sorted(fam_results.items(), key=lambda x: x[1], reverse=True)[:5]:
        print(f"{fam}: {int(val)}")

    return int(total_sales)

# =============================================================================
# MANUAL INPUTS (UPDATED)
# =============================================================================
"""
if __name__ == "__main__":
    
    # >>> UPDATE VALUES HERE <<<
    predict_single_day_manual(
        target_date_str="2026-05-15",
        temperature=18.5,          # Avg Daily Temperature
        rain_morning=0.0,          # Rain mm (00:00 - 12:00)
        rain_afternoon=2.5,        # Rain mm (12:00 - 24:00)
        precip_morning=0.0,        # Precip mm (00:00 - 12:00)
        precip_afternoon=3.0,      # Precip mm (12:00 - 24:00)
        event_name=None,           # e.g., "formula_1"
        holiday_name=None,         # e.g., "Christmas"
        holiday_intensity=0        # 0 to 1
    )
"""

...
def process_data():
    ...
    try:
        ...
        print(f"Error loading campaign data: {e}")
        recurring_og_df = pd.DataFrame()

    # basic checks for empty dataframes and correct columns
    # 0 = visitor, 1 = weather, 2 = holiday, 3 = campaings, 4 = recurring events
    dfs = [visitor_og_df, weather_og_df, holiday_og_df, camp_og_df, recurring_og_df]
    ...
    events_dtypes = {
        "Events": lambda dt: dt == 'object',
        "Datum": ptypes.is_datetime64_any_dtype,
    }
    for col, check_func in events_dtypes.items():
        if col not in recurring_og_df.columns or not check_func(recurring_og_df[col].dtype):
            print(f"Event DataFrame column '{col}' has incorrect dtype: {recurring_og_df[col].dtype if col in recurring_og_df else 'missing'}")
            return
    ...
    # expand to include 0 sales on all days if it wasnt listed
    ...
    merged_df['event_name'] = merged_df['event_name'].fillna('no_event') # This now works correctly
    ...
    # one hot encode both categorical columns here
    ...
    print("✅ Data processing complete. File saved to ../data/processed/processed_merge.csv")
